...
export default function Question8({ navigation, route }) {
  ...
  const { isRecording, isTranscribing, startRecording, stopRecording } = useVoiceInput((text) => {
    ...
    if (t.includes('daily')) setSelected('daily');
    else if (t.includes('week') || t.includes('weekly')) setSelected('weekly');
    else if (t.includes('less') || t.includes('rare')) setSelected('less');
  });
  ...
  const options = [
    ...
    { id: "less", emoji: "📬", label: "Less often" },
  ];

  const handleNext = () => {
    if (!selected) {
      Alert.alert("Please select an option", "Choose how often you connect with others.");
      return;
    }
    navigation.navigate("Question9", {
      questionnaireData: { ...questionnaireData, socialFrequency: selected }
    });
  };
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.progressContainer}>
          ...
          <Text style={styles.progressText}>Question 8 of 9</Text>
        </View>
        ...
        <Text style={styles.question}>How often do you see or talk to family/friends?</Text>

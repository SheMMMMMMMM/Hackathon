...
export default function Question4({ navigation, route }) {
  ...
  const { isRecording, isTranscribing, startRecording, stopRecording } = useVoiceInput((text) => {
    ...
    if (t.includes('yes') || t.includes('every') || t.includes('daily')) setSelected('everyday');
    else if (t.includes('some') || t.includes('sometimes')) setSelected('sometimes');
    else if (t.includes('no') || t.includes('none')) setSelected('no');
  });
  ...
  const options = [
    { id: "everyday", emoji: "💊", label: "Yes, every day" },
    { id: "sometimes", emoji: "⏰", label: "Sometimes" },
  ...
  const handleNext = () => {
    ...
    navigation.navigate("Question5", {
      questionnaireData: { ...questionnaireData, dailyMedications: selected }
    });
  };
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.progressContainer}>
          ...
          <Text style={styles.progressText}>Question 4 of 9</Text>
        </View>
        ...
        <Text style={styles.question}>Do you take any medications on a daily basis?</Text>

...
export default function Question5({ navigation, route }) {
  ...
  const { isRecording, isTranscribing, startRecording, stopRecording } = useVoiceInput((text) => {
    ...
    if (t.includes('alone')) setSelected('alone');
    else if (t.includes('family')) setSelected('family');
    else if (t.includes('care') || t.includes('caregiver')) setSelected('caregiver');
  });
  ...
  const options = [
    { id: "alone", emoji: "🧓", label: "I live alone" },
    { id: "family", emoji: "👨‍👩‍👧", label: "With family" },
    { id: "caregiver", emoji: "👩‍⚕️", label: "With a caregiver" },
  ];

  const handleNext = () => {
    if (!selected) {
      Alert.alert("Please select an option", "Choose your living arrangement.");
      return;
    }
    navigation.navigate("Question6", {
      questionnaireData: { ...questionnaireData, livingArrangement: selected }
    });
  };
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.progressContainer}>
          ...
          <Text style={styles.progressText}>Question 5 of 9</Text>
        </View>
        ...
        <Text style={styles.question}>Do you live alone or with family?</Text>

...
export default function Question9({ navigation, route }) {
  const [selected, setSelected] = useState(null);
  const { questionnaireData = {} } = route.params || {};
  const { completeQuestionnaire } = useUser();
  const { isRecording, isTranscribing, startRecording, stopRecording } = useVoiceInput((text) => {
    if (!text) return;
    const t = text.toLowerCase();
    if (t.includes('touch')) setSelected('touch');
    else if (t.includes('voice') && !t.includes('both')) setSelected('voice');
    else if (t.includes('both')) setSelected('both');
  });
  ...
  const handleNext = () => {
    if (!selected) {
      Alert.alert("Please select an option", "Choose your preferred interaction method.");
      return;
    }
    
    // Save complete questionnaire data
    const finalData = { ...questionnaireData, preferredInteraction: selected };
    completeQuestionnaire(finalData);
    
    Alert.alert(
      "Setup Complete! 🎉",
      "Thank you for completing the setup. Let's get started!",
      [
        {
          text: "Let's Go!",
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.progressContainer}>
          ...
          <Text style={styles.progressText}>Question 9 of 9</Text>
        </View>
        ...
        <Text style={styles.question}>How do you prefer to interact with the app?</Text>

...
export default function Question6({ navigation, route }) {
  ...
  const { isRecording, isTranscribing, startRecording, stopRecording } = useVoiceInput((text) => {
    ...
    if (t.includes('no') || t.includes('easily')) setSelected('no');
    else if (t.includes('some') || t.includes('difficulty')) setSelected('some');
    else if (t.includes('yes') || t.includes('assist') || t.includes('help')) setSelected('yes');
  });
  ...
  const options = [
    { id: "no", emoji: "💪", label: "No, I move easily" },
    { id: "some", emoji: "🦯", label: "Some difficulty" },
    { id: "yes", emoji: "♿", label: "Yes, I use assistance" },
  ];

  const handleNext = () => {
    if (!selected) {
      Alert.alert("Please select an option", "Choose your mobility level.");
      return;
    }
    navigation.navigate("Question7", {
      questionnaireData: { ...questionnaireData, mobilityDifficulties: selected }
    });
  };
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.progressContainer}>
          ...
          <Text style={styles.progressText}>Question 6 of 9</Text>
        </View>
        ...
        <Text style={styles.question}>Do you have any mobility difficulties?</Text>
